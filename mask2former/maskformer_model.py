    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            if "instances" in batched_inputs[0]:
                ...
                targets = None
            coarse_prediction = outputs["coarse_masks"]
            ...
            # # for coarse prediction

            for i in range(len(targets)):
                t_mask = targets[i]["masks"]  # N_I, H, W
                targets[i]["masks"] = (
                    F.interpolate(
                        t_mask[:, None].to(torch.float32),
                        coarse_prediction.shape[2:],
                        mode="nearest",
                    )
                    .type(torch.int64)
                    .squeeze(1)
                )
            outputs["pred_logits"] = outputs["coarse_logits"]
            outputs["pred_masks"] = outputs["coarse_masks"]
            del outputs["aux_outputs"]

            coarse_losses = self.criterion(outputs, targets)
            for k in list(coarse_losses.keys()):
                if k in self.criterion.weight_dict:
                    coarse_losses[k + "_coarse"] = (
                        0.1 * coarse_losses[k] * self.criterion.weight_dict[k]
                    )
                    del coarse_losses[k]
                else:
                    # remove this loss if not specified in `weight_dict`
